    ...
    return (
      <div key={item.path} className="flex flex-row items-center gap-2 text-sm">
        ...
        <div className="flex flex-col gap-1">
          <p className="text-xs">From: {item.path}</p>
          <Code
            code={getImportCode(item)}
            language="js"
            customStyle={{
              paddingTop: 0,
              paddingBottom: 0,
...
const getImportCode = (item: ModuleTreeItem) => {
  switch (item.referenceType) {
    case 'dynamic-import':
      return `import('${item.original}')`;

    case 'import-statement':
      return `import '${item.original}'`;

    case 'require-call':
      return `require('${item.original}')`;
